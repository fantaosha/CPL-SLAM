...
SESyncProblem::SESyncProblem(
  ...
  d_ = (!measurements.first.empty() ? measurements.first[0].t.size() : 0);
  r_ = d_;
    ...
    if (preconditioner_ == Preconditioner::Jacobi) {
      ...
      if (n_[1] == 0) {
        ...
        reg_Chol_precon_.compute(
            LGrho_ + SparseMatrix(Vector::Constant(
                                      LGrho_.rows(),
        ...
        reg_Chol_precon_.compute(
            M_ +
            SparseMatrix(Vector::Constant(
                             M_.rows(), lambda_max / reg_Chol_precon_max_cond_)
                             .asDiagonal()));
      }
    }
  } else {
...
Matrix SESyncProblem::Riemannian_Hessian_vector_product(
    const Matrix &Y, const Matrix &dotY) const {
...
Matrix SESyncProblem::precondition(const Matrix &Y, const Matrix &dotY) const {
  if (preconditioner_ == Preconditioner::None)
    ...
    if (n_[1] == 0)
      ...
      Matrix X = reg_Chol_precon_.solve(Z.transpose())
                     .bottomRows(d_ * n_[0])
                     .transpose();
...
Matrix SESyncProblem::round_solution(const Matrix Y) const {
  ...
  Vector determinants(n_[0]);

  // Compute the offset at which the rotation matrix blocks begin
  size_t rot_offset = (form_ == Formulation::Simplified ? 0 : size_);
...
#pragma omp parallel for
  ...
  else  // form == Explicit
  {
    ...
    Matrix X(d_, (d_ + 1) * n_[0] + n_[1]);

    // Set rotational states
    X.block(0, size_, d_, d_ * n_[0]) = R;

    // Recover translational states
    X.block(0, 0, d_, size_) = recover_landmarks_and_translations(B1_, B2_, R);
...
Matrix SESyncProblem::compute_Lambda_blocks(const Matrix &Y) const {
  ...
  Matrix Lambda_blocks(d_, n_[0] * d_);

  // Index of the row/column at which the rotational blocks begin in matrix X
  size_t offset = (form_ == Formulation::Simplified ? 0 : size_);
...
SparseMatrix SESyncProblem::compute_Lambda_from_Lambda_blocks(
  ...
  SparseMatrix Lambda(offset + d_ * n_[0], offset + d_ * n_[0]);
...
bool SESyncProblem::compute_S_minus_Lambda_min_eig(
  ...
  Spectra::SymEigsSolver<Scalar, Spectra::SELECT_EIGENVALUE::LARGEST_MAGN,
      ...
      largest_magnitude_eigensolver(&lm_op, 1,
                                    std::min(num_Lanczos_vectors, n_[0] * d_));
  ...
  Vector xinit = v0 + (.01 * v0.norm()) * perturbation;  // Perturb v0 by ~3%
...
Matrix SESyncProblem::chordal_initialization() const {
  Matrix Y;
  if (form_ == Formulation::Simplified) {
    Y = Matrix::Zero(r_, n_[0] * d_);
    if (B1_.cols() * d_ == B3_.cols())
      Y.topRows(d_) = SESync::chordal_initialization(d_, B3_);
    else
      Y.topRows(d_) = SESync::chordal_initialization(d_, B1_, B2_, B3_);
  } else  // form == explicit
  {
    Y = Matrix::Zero(r_, n_[0] * (d_ + 1) + n_[1]);

    // Compute rotations using chordal initialization
    if (B1_.cols() * d_ == B3_.cols())
      Y.block(0, size_, d_, n_[0] * d_) =
          SESync::chordal_initialization(d_, B3_);
    else
      Y.block(0, size_, d_, n_[0] * d_) =
          SESync::chordal_initialization(d_, B1_, B2_, B3_);

    // Recover corresponding translations
    Y.block(0, 0, d_, size_) = recover_landmarks_and_translations(
        B1_, B2_, Y.block(0, size_, d_, n_[0] * d_));
  }
...
Matrix SESyncProblem::random_sample() const {
  ...
  else  // form == Explicit
  {
    ...
    Y.block(0, size_, r_, n_[0] * d_) = SP_.random_sample();
    ...
    for (size_t i = 0; i < r_; ++i)
      for (size_t j = 0; j < size_; ++j) Y(i, j) = g(generator);
  }
