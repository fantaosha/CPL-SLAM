...
// Set number of threads
  ...
  for (size_t r = options.r0; r <= options.rmax; r++) {
    ...
    SESyncResults.elasped_optimization_time += TNTResults.elapsed_time;
  ...
  if (options.verbose) {
    ...
    switch (SESyncResults.status) {
      ...
      case EIG_IMPRECISION:
        std::cout
            << "WARNING: Minimum eigenvalue computation did not achieve "
  ...
  SESyncResults.Fxhat =
      (options.formulation == Formulation::Simplified
           ? problem.evaluate_objective(SESyncResults.xhat.block(
                 0, problem.num_poses() + problem.num_landmarks(), problem.dimension(),
