/** This file provides a convenient functional interface to the CPL-SLAM
 * algorithm.
 *
...
/** This struct contains the various parameters that control the CPL-SLAM
 * algorithm */
struct CPL_SLAMOpts {
  ...
  Retraction retraction = Retraction::Projection;

  /** The specific formulation of the CPL-SLAM problem to solve */
   ...
   * initialization strategy employed by CPL-SLAM: 'true' -> chordal, 'false' ->
   * random sampling */
  ...
  /** If this value is true, the CPL-SLAM algorithm will log and return the
   * entire sequence of iterates generated by the Riemannian Staircase */
  bool log_iterates = false;

#if defined(_OPENMP)
  /** The number of threads to use for parallelization (assuming that CPL-SLAM
   * is
...
/** These enumerations describe the termination status of the CPL-SLAM algorithm
 */
enum CPL_SLAMStatus {
...
/** This struct contains the output of the CPL-SLAM algorithm */
struct CPL_SLAMResult {
  /** An estimate of a global minimizer Yopt of the rank-restricted dual
   * semidefinite relaxation Problem 9 in the CPL-SLAM tech report.  The
  ...
  /** The total elapsed computation time for the CPL-SLAM algorithm */
  ...
  double elapsed_optimization_time = 0;
  ...
  /** If CPL-SLAM was run with log_iterates = true, this will contain the
   * sequence of iterates generated by the truncated-Newton trust-region method
   * at each level of the Riemannian Staircase */
  std::vector<std::vector<ComplexMatrix>> iterates;
...
CPL_SLAMResult CPL_SLAM(CPL_SLAMProblem &problem,
                      const CPL_SLAMOpts &options = CPL_SLAMOpts(),
                      const ComplexMatrix &Y0 = ComplexMatrix());

/** Given a vector of relative pose measurements specifying a special Euclidean
 * synchronization problem, performs synchronization using the CPL-SLAM algorithm
 */
